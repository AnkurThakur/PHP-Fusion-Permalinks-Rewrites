...
if (isset($_POST['savepermalinks'])) {
	...
	if (isset($_POST['permalink']) && is_array($_POST['permalink'])) {
		...
		foreach ($permalinks as $key=>$value) {
			$result = dbquery("UPDATE ".DB_PREFIX."permalinks_patterns SET pattern_source='".$value."' WHERE pattern_id='".$key."'");
			if (!$result) {	$error = 1;		}
		}
	}
	else {
	...
	if ($error == 0) {
		echo "<div id='close-message'><div class='admin-message'>".$locale['421']."</div></div>\n";
	}
	elseif($error == 1) {
...
if (isset($_GET['edit']) && file_exists(INCLUDES."rewrites/".stripinput($_GET['edit'])."_rewrite_include.php")) {
	...
	$rows = dbcount("(rewrite_id)", DB_PREFIX."permalinks_rewrites", "rewrite_name='".$rewrite_name."'");

	if ($rows>0) {
		$result = dbquery("SELECT p.* FROM ".DB_PREFIX."permalinks_rewrites r INNER JOIN ".DB_PREFIX."permalinks_patterns p ON r.rewrite_id=p.pattern_type WHERE r.rewrite_name='".$rewrite_name."'");

		if (dbrows($result)) {
			opentable(sprintf($locale['405'], $permalink_name));

			echo "<form name='editpatterns' method='post' action='".FUSION_SELF.$aidlink."'>\n";
			echo "<table cellpadding='0' cellspacing='1' width='60%' class='tbl-border center'>\n";
			...
			echo "<tr>\n";
			echo "<td class='tbl2' style='white-space:nowrap'><strong>".$locale['408']."</strong></td>\n";
			echo "<td class='tbl2' style='white-space:nowrap'><strong>".$locale['409']."</strong></td>\n";
			echo "</tr>\n";
			...
			echo "<td class='tbl2' style='white-space:nowrap'><input type='submit' value='".$locale['413']."' class='button' name='savepermalinks' /></td>\n";
			echo "</tr>\n";
			...
			echo "<div id='close-message'><div class='admin-message'>".sprintf($locale['422'], $permalink_name)."</div></div>\n";
		}
	}
	else {
		echo "<div id='close-message'><div class='admin-message'>".$locale['423']."</div></div>\n";
	}
	...
	if ($rows==0) {
		...
		$result = dbquery("INSERT INTO ".DB_PREFIX."permalinks_rewrites (rewrite_name) VALUES ('".$rewrite_name."')");
		if (!$result) {	$error = 1;		}
		...
		if (isset($pattern) && is_array($pattern)) {
			foreach ($pattern as $source=>$target) {
				$result = dbquery("INSERT INTO ".DB_PREFIX."permalinks_patterns (pattern_type, pattern_source, pattern_target, pattern_cat) VALUES ('".$last_insert_id."', '".$source."', '".$target."', 'normal')");
				if (!$result) {	$error = 1;		}
			}
		}
		if (isset($alias_pattern) && is_array($alias_pattern)) {
			foreach ($alias_pattern as $source=>$target) {
				$result = dbquery("INSERT INTO ".DB_PREFIX."permalinks_patterns (pattern_type, pattern_source, pattern_target, pattern_cat) VALUES ('".$last_insert_id."', '".$source."', '".$target."', 'alias')");
				if (!$result) {	$error = 1;		}
			}
		}

		if ($error == 0) {
			echo "<div id='close-message'><div class='admin-message'>".sprintf($locale['424'], $permalink_name)."</div></div>\n";
		}
		elseif($error == 1) {
			echo "<div id='close-message'><div class='admin-message'>".$locale['420']."</div></div>\n";
		}
		...
		echo "<div id='close-message'><div class='admin-message'>".sprintf($locale['425'], $permalink_name)."</div></div>\n";
	}
	...
	// Delete Data
	$rewrite_id = dbarray(dbquery("SELECT rewrite_id FROM ".DB_PREFIX."permalinks_rewrites WHERE rewrite_name='".$rewrite_name."' LIMIT 1"));
	$result = dbquery("DELETE FROM ".DB_PREFIX."permalinks_rewrites WHERE rewrite_id=".$rewrite_id['rewrite_id']);
	$result = dbquery("DELETE FROM ".DB_PREFIX."permalinks_patterns WHERE pattern_type=".$rewrite_id['rewrite_id']);

	if ($result) {
		echo "<div id='close-message'><div class='admin-message'>".sprintf($locale['426'], $permalink_name)."</div></div>\n";
	}
}
...
$result = dbquery("SELECT * FROM ".DB_PREFIX."permalinks_rewrites	ORDER BY rewrite_name ASC");
...
if (dbrows($result)) {
	...
	while ($data = dbarray($result)) {
		...
		echo "<tr>\n";
		if (!file_exists(INCLUDES."rewrites/".$data['rewrite_name']."_rewrite_include.php") ||
		!file_exists(INCLUDES."rewrites/".$data['rewrite_name']."_rewrite_info.php") ||
		!file_exists(LOCALE.LOCALESET."permalinks/".$data['rewrite_name'].".php")) {
...
if (count($available_rewrites) != count($enabled_rewrites)) {
	...
	for ($k = 0; $k < count($available_rewrites); $k++) {
		if (!in_array($available_rewrites[$k], $enabled_rewrites)) {
			if (file_exists(LOCALE.LOCALESET."permalinks/".$available_rewrites[$k].".php")) {
				include LOCALE.LOCALESET."permalinks/".$available_rewrites[$k].".php";
			}
			if (file_exists(INCLUDES."rewrites/".$available_rewrites[$k]."_rewrite_info.php")) {

...
class Rewrite
{
	...
	*/
	...
	/*
	* Show Warning related to Rewrite or not
	* @data_type Boolean
	* @access private
	*/
	private $show_warnings = false;
	...
	* Constructor
	*
	* Cleans the Request URI for matching with patterns
	* @access public
	*/
	public function __construct()
	{
		...
		* Example: If Request_URI is '/infusions/infusion/event-5-basketball', then clean URI will be 'event-5-basketball'
		*/
		global $settings;
	...
	*/
	...
	{
		$query = "SELECT rewrite_name FROM ".DB_PREFIX."permalinks_rewrites";
	...
	*/
	private function checkAlias()
	{
		...
		$query = "SELECT * FROM ".DB_PREFIX."permalinks_alias WHERE alias_url='".$this->requesturi."' LIMIT 1";
		$result = dbquery($query);
		$this->queries[] = $query;
				...
				foreach ($alias_patterns as $search=>$replace) {
					...
					if (preg_match($search, $this->requesturi, $matches)) {
						...
						if ($alias_pos != 0)	{
							...
							$query = "SELECT * FROM ".DB_PREFIX."permalinks_alias WHERE alias_url='".$alias."' LIMIT 1";
							$result = dbquery($query);
							$this->queries[] = $query;
...
/*
	...
	*/
	private function setWarning($code, $info="")
	{
		$info = ($info != "") ? $info." : " : "";
		$warnings = array(
			1 => "No matching Alias found.",
			2 => "No matching Alias Pattern found.",
			3 => "No matching Regex pattern found.",
			4 => "Rewrite Include file not found.",
			5 => "Tag not found in the pattern.",
			6 => "File path is empty.",
			7 => "Alias found.",
			8 => "Alias Pattern found.",
			9 => "Regex Pattern found."
		);
		if ($code<=6) {
